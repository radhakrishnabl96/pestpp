...
import pandas as pd
import os
from pestools import plots
from pestools.mat_handler import jco as Jco
from pestools.pst_handler import pst as Pst
    ...
    def group(self, group, n=None):
        ...
        if n_head > 0:
            sensitivity = self.df.sort_values(by='Sensitivity',
                                       ascending = False).loc[self.df['Parameter Group'] == group].head(n=n_head)
        if n_head < 0:
            n_head = abs(n_head)
            sensitivity = self.df.sort_values(by='Sensitivity',
                                       ascending = False).loc[self.df['Parameter Group'] == group].tail(n=n_head)
