...
class Hist(Plot):
    ...
    def _make_plot(self):
        ...
        hist_df = self.df.loc[self.df.Group.isin(self.groups), [self.by, self.values]]
        ...
        for i, sp in enumerate(self.subplots):
            ...
            g = self.df.loc[inds, self.values]
...
class SpatialPlot(ScatterPlot):
    ...
    def __init__(self, df, x, y, values, groupinfo,
        ...
        **kwds:
        ...
        self.scatter_df = self.df.loc[self.df[self.group_col].isin(self.groups)]
...
class IdentBar(Plot):
    ...
    def _make_plot(self):
        ...
        b = self._df_Nvalues.loc[0:self.nbars][self._df_Nvalues.columns[:-1]].plot(ax=axmain, kind='bar', stacked=True, width=0.8, colormap='jet_r')
        b.legend_ = None
